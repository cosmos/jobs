<?php
/**
 * Front Meta Box Functions
 *
 * @author      MadrasThemes
 * @category    Core
 * @package     FrontExtensions/Admin/Functions
 * @version     2.3.0
 */
if ( ! defined( 'ABSPATH' ) ) {
 ...
 * @param array $field
 */
function front_wp_text_input( $field ) {
    ...
    switch ( $data_type ) {
        case 'decimal':
            ...
            $field['value']  = front_format_localized_decimal( $field['value'] );
            break;
        case 'url':
    ...
    if ( ! empty( $field['description'] ) && false !== $field['desc_tip'] ) {
        echo front_help_tip( $field['description'] );
    }
 ...
 * @param array $field
 */
function front_wp_hidden_input( $field ) {
    global $thepostid, $post;
 ...
 * @param array $field
 */
function front_wp_textarea_input( $field ) {
    global $thepostid, $post;
 ...
 * @param array $field
 */
function front_wp_checkbox( $field ) {
    global $thepostid, $post;
 ...
 */
function front_wp_select( $field ) {
    global $thepostid, $post;
    ...
    ?>
    <p <?php echo front_implode_html_attributes( $wrapper_attributes ); // WPCS: XSS ok. ?>>
        <label <?php echo front_implode_html_attributes( $label_attributes ); // WPCS: XSS ok. ?>><?php echo wp_kses_post( $field['label'] ); ?></label>
        <?php if ( $tooltip ) : ?>
            <?php echo front_help_tip( $tooltip ); // WPCS: XSS ok. ?>
        <?php endif; ?>
        <select <?php echo front_implode_html_attributes( $field_attributes ); // WPCS: XSS ok. ?>>
            <?php
            foreach ( $field['options'] as $key => $value ) {
                echo '<option value="' . esc_attr( $key ) . '"' . front_selected( $key, $field['value'] ) . '>' . esc_html( $value ) . '</option>';
            }
            ?>
        </select>
        <?php if ( $description ) : ?>
