 ...
 */
class Front_WPJM_Widget_Layered_Nav extends WP_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $widget_ops = array( 'description' => esc_html__( 'Add job filter widgets to your sidebar.', 'front-extensions' ) );
        parent::__construct( 'front_wpjm_layered_nav', esc_html__( 'Front Filter Job by Taxonomy', 'front-extensions' ), $widget_ops );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $_chosen_taxonomies = Front_WPJM_Query::get_layered_nav_chosen_taxonomies();
        ...
        $get_terms_args = apply_filters( 'front_wpjm_layered_nav_terms_args', array( 'hide_empty' => '1' ) );
    ...
    protected function get_filtered_term_job_counts( $term_ids, $taxonomy, $query_type ) {
        ...
        $tax_query  = Front_WPJM_Query::get_main_tax_query();
        $meta_query = Front_WPJM_Query::get_main_meta_query();
        $date_query = Front_WPJM_Query::get_main_date_query();
        ...
        $query             = apply_filters( 'front_wpjm_get_filtered_term_job_counts_query', $query );
        $query             = implode( ' ', $query );
        ...
        $cache = apply_filters( 'front_wpjm_layered_nav_count_maybe_cache', true );
        if ( true === $cache ) {
            $cached_counts = (array) get_transient( 'front_wpjm_layered_nav_counts_' . $taxonomy );
        } else {
            $cached_counts = array();
        }
        ...
        if ( ! isset( $cached_counts[ $query_hash ] ) ) {
            ...
            if ( true === $cache ) {
                set_transient( 'front_wpjm_layered_nav_counts_' . $taxonomy, $cached_counts, DAY_IN_SECONDS );
            }
        }
    ...
    protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
        // List display.
        echo '<div class="front-wpjm-widget-layered-nav-list tax-' . esc_attr( $taxonomy ) . '">';
        ...
        foreach ( $terms as $term ) {
            ...
            $show_hide_limit = apply_filters( "front_wpjm_layered_nav_show_hide_limit_{$taxonomy}", 4 );

            $count_html = apply_filters( 'front_wpjm_layered_nav_count', '<small class="count">' . absint( $count ) . '</small>', $count, $term );

            if ( $count > 0 || $option_is_set ) {
                $term_uniqid = $term->slug . '-' . uniqid();
                $link      = esc_url( apply_filters( 'front_wpjm_layered_nav_link', $link, $term, $taxonomy ) );
            ...
            echo '<div class="form-group d-flex align-items-center justify-content-between font-size-1 text-lh-md text-secondary mb-2 front-wpjm-widget-layered-nav-list__item front-wpjm-layered-nav-term">';
            echo apply_filters( 'front_wpjm_layered_nav_term_html', $term_html, $term, $link, $count );
            echo '</div>';
