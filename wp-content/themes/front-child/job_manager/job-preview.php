...
<form method="post" id="job_preview" action="<?php echo esc_url( $form->get_action() ); ?>">
	...
	?>
	<div class="job_posting_preview_title">
		<h2><?php echo( '<span class="btn btn-xs btn-soft-primary">'.get_the_title().'\'s  job preview</span>'); ?></h2>
		<input type="submit" name="edit_job" class="button" value="<?php esc_attr_e( '&larr; Edit listing', 'wp-job-manager' ); ?>" />
		<input type="submit" name="continue" id="job_preview_submit_button" class="button job-manager-button-submit-listing" value="<?php echo esc_attr( apply_filters( 'submit_job_step_preview_submit_text', __( 'Submit Listing &rarr;', 'wp-job-manager' ) ) ); ?>" />
	</div>
